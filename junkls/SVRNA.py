...
\t This pipeline is composed by  28 steps check if running all
\t example use are this:
\t ~/software/SVRNA.py -sample 116_sort.bam -step 01 -ID 116

\t where -sample is the name of the file -step 01 is where they start and the ID are the tag of all names:
...
if '-step' in args:
	...
	step= "01"	
bampath=os.path.abspath(inputpath)
...
step_20_cmd=['/illumina/software/PY276/bin/bam_stat.py -i %s/%s  &> out/%sstatistics.txt'%(bampath,sample,ID)]

step_21_cmd=['/illumina/software/PY276/bin/junction_annotation.py -i %s/%s.sorted.bam.bam -r /illumina/software/database/RSeqc/hg19_RefSeq.bed  -o %s &> out/%s_annotaion.juction.txt'%(bampath,sample,ID,ID)]
...
for i in range(0,len(cmdset)):
    cmd=cmdset[i]
    if float(cmd.split("_")[1]) >= float(step):
	clean_flag=1
